(function () {
  ...
  function render() {
    ...
    $('blockList').innerHTML = state.blocks.length ? state.blocks.map((block, index) => `<div class="block-row ${index === step - 1 ? 'active' : ''}"><b>${block.label}</b><span>值 ${block.value}</span><span>让 ${block.handout}</span><span>${block.short ? '<i class="badge short">短块</i>' : '<i class="badge">普通块</i>'} ${block.controlLabel}</span></div>`).join('') : '<p>点击“分析下一块”。</p>'
