(function (root, factory) {
  ...
  const edgeText = edgeIndex => {
    const edge = edges[edgeIndex]
    const related = edge.boxes.map(box => boxes[box].label)
    const human = related.length === 2 ? `${related[0]}–${related[1]} 公共边` : related.length === 1 ? `${related[0]} 外边` : '缺角边'
    const end = edge.dir === 'H'
      ? `(${edge.c},${edge.r})-(${edge.c + 1},${edge.r})`
      : `(${edge.c},${edge.r})-(${edge.c},${edge.r + 1})`
  ...
  return { N, edges, boxes, edgeById, FULL_MASK, has, put, bitCount, boxComplete, degrees, legal, gain, claimedBy, danger, edgeText, connectedBoxes }
})
